...
export default function AddProperty() {
  ...
  const [form, setForm] = useState({
    ...
    distanceToStadium: '', amenities: [] as string[],
  })

  const createProperty = trpc.property.create.useMutation({
    onSuccess: (data) => {
      setSuccess('Property "' + data.title + '" submitted! It will appear after admin approval.')
      setTimeout(() => navigate('/host'), 2500)
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.title.trim()) { setError('Property name is required'); return }
    ...
    createProperty.mutate({
      ...
      images: JSON.stringify(images.length > 0 ? images : ['https://images.unsplash.com/photo-1522708323590-d24dbb6b0267']),
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-midnight rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <ImageUpload images={images} videos={videos} onImagesChange={setImages} onVideosChange={setVideos} />
              {images.length === 0 && <p className="text-gray-500 text-xs mt-1">A default image will be used if none uploaded</p>}
            </div>
            ...
            <div>
              ...
              <textarea required value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3} className="w-full bg-deep-forest border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-sunset focus:outline-none" placeholder="Describe your property, neighborhood, what makes it special..." />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Price/Night (UGX) *</label>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div><label className="block text-sm font-medium text-gray-300 mb-1">Capacity</label><input type="number" min="1" value={form.capacity} onChange={e => setForm(f => ({ ...f, capacity: e.target.value }))} className="w-full bg-deep-forest border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-sunset focus:outline-none" /></div>
              <div><label className="block text-sm font-medium text-gray-300 mb-1">Bedrooms</label><input type="number" min="0" value={form.bedrooms} onChange={e => setForm(f => ({ ...f, bedrooms: e.target.value }))} className="w-full bg-deep-forest border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-sunset focus:outline-none" /></div>
              <div><label className="block text-sm font-medium text-gray-300 mb-1">Bathrooms</label><input type="number" min="0" value={form.bathrooms} onChange={e => setForm(f => ({ ...f, bathrooms: e.target.value }))} className="w-full bg-deep-forest border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-sunset focus:outline-none" /></div>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Distance to Stadium</label>
              <input type="text" value={form.distanceToStadium} onChange={e => setForm(f => ({ ...f, distanceToStadium: e.target.value }))} className="w-full bg-deep-forest border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-sunset focus:outline-none" placeholder="e.g. 2.1 km to Mandela Stadium" />
            </div>
            ...
            <div className="flex gap-6">
              <label className="flex items-center gap-2 text-gray-300 cursor-pointer"><input type="checkbox" checked={form.hasShuttle} onChange={e => setForm(f => ({ ...f, hasShuttle: e.target.checked }))} className="w-4 h-4" /> Stadium Shuttle</label>
              <label className="flex items-center gap-2 text-gray-300 cursor-pointer"><input type="checkbox" checked={form.isGroupFriendly} onChange={e => setForm(f => ({ ...f, isGroupFriendly: e.target.checked }))} className="w-4 h-4" /> Group Friendly</label>
            </div>
