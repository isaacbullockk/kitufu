...
async function checkAvailabilityConflict(propertyId: number, checkIn: string, checkOut: string): Promise<number> {
  const db = getDb();
  const bookedDates = await db.select({ count: sql<number>`count(*)` }).from(availability).where(
    and(eq(availability.propertyId, propertyId), eq(availability.isBooked, 1), gte(availability.date, checkIn), lte(availability.date, checkOut))
  );
  const overlappingBookings = await db.select({ count: sql<number>`count(*)` }).from(bookings).where(
    and(eq(bookings.propertyId, propertyId), sql`${bookings.status} IN (\'pending\', \'confirmed\')`, gte(bookings.checkOut, checkIn), lte(bookings.checkIn, checkOut))
  );
  return (bookedDates[0]?.count || 0) + (overlappingBookings[0]?.count || 0);
}

async function blockAvailability(propertyId: number, checkIn: string, checkOut: string, bookingId: number): Promise<void> {
  const db = getDb();
  const start = new Date(checkIn);
  const end = new Date(checkOut);
  for (let d = new Date(start); d < end; d.setDate(d.getDate() + 1)) {
    const dateStr = d.toISOString().split("T")[0];
    await db.insert(availability).values({ propertyId, date: dateStr, isBooked: 1, bookingId }).onDuplicateKeyUpdate({ set: { isBooked: 1, bookingId } });
  }
}
...
export const bookingRouter = createRouter({
  create: publicQuery
    ...
    .mutation(async ({ input }) => {
      ...
      if (checkInDate < today) throw new TRPCError({ code: "BAD_REQUEST", message: "Check-in date cannot be in the past" });
      if (checkOutDate <= checkInDate) throw new TRPCError({ code: "BAD_REQUEST", message: "Check-out date must be after check-in date" });
  ...
  listByHost: publicQuery.input(z.object({ hostId: z.number() })).query(async ({ input }) => {
    try {
      const db = getDb();
      ...
      return db.select().from(bookings).where(sql`${bookings.propertyId} IN (${propertyIds.join(",")})`).orderBy(desc(bookings.checkIn));
  ...
  cancel: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    ...
    if (existing.length === 0) throw new TRPCError({ code: "NOT_FOUND", message: "Booking " + input.id + " not found" });
  ...
  confirm: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    const existing = await db.select().from(bookings).where(eq(bookings.id, input.id)).limit(1);
    if (existing.length === 0) throw new TRPCError({ code: "NOT_FOUND", message: "Booking " + input.id + " not found" });
  ...
  byRef: publicQuery.input(z.object({ bookingRef: z.string() })).query(async ({ input }) => {
    ...
    if (result.length === 0) throw new TRPCError({ code: "NOT_FOUND", message: "Booking " + input.bookingRef + " not found" });
    return result[0];
  }),
});
