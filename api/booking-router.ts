...
async function blockAvailability(
  ...
  for (let d = new Date(start); d < end; d.setDate(d.getDate() + 1)) {
    ...
    await db
      .insert(availability)
      .values({ propertyId, date: dateStr, isBooked: 1, bookingId })
...
export const bookingRouter = createRouter({
  create: publicQuery
    ...
    .mutation(async ({ input }) => {
      ...
      if (checkOutDate <= checkInDate) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Check-out date must be after check-in date" });
      }
      if (input.adults + input.children > 20) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Maximum 20 guests per booking" });
      }
      ...
      if (property.length === 0) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Property " + input.propertyId + " not found" });
      }
      ...
      if (conflicts > 0) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "This property is not available for the selected dates (" + conflicts + " conflicting booking(s))",
        });
      }
  ...
  listByUser: publicQuery
    ...
    .query(async ({ input }) => {
      try {
        const db = getDb();
        return db.select().from(bookings).where(eq(bookings.userId, input.userId)).orderBy(desc(bookings.checkIn));
      } catch {
  ...
  listByHost: publicQuery
    ...
    .query(async ({ input }) => {
      try {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to fetch host bookings" });
      }
    }),
